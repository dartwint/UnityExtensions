...
namespace Dartwint.UnityExtensions.IOPackageManager
{
    ...
    public class FilePickerMediatorPanelViewModel
    {
        ...
        public FilePickerMediatorPanelViewModel(FilePickerMediatorPanelModel model, Package package)
        {
            this.model = model;
            _package = package;
        }
        ...
        public void SaveChanges()
        {
            ...
            if (PackageFilesEditMode == PackageFilesEditMode.ADD)
                _package.packageFiles.AddFiles(files);
            else if (PackageFilesEditMode == PackageFilesEditMode.REMOVE)
                _package.packageFiles.RemoveFiles(files);

            PackageManager.Save(_package);
        }
